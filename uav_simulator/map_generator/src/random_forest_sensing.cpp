...
double _oneway_time;
int _dynamic_obs_vel = 1; // m/s
double _dynamic_obs_x;
double _dynamic_obs_y;
double _dynamic_obs_z;
double _dynamic_obs_width = 1.0;
double _dynamic_obs_height = 0.2;
...
void DynamicObsGenerate() {
  ...
  int widNum = ceil(_dynamic_obs_width / _resolution);
  int heiNum = ceil(_dynamic_obs_height / _resolution);

  _dynamic_obs_y += _delta_time*_dynamic_obs_vel;
  ...
  for (int r = -widNum / 2.0; r < widNum / 2.0; r++)
    for (int s = -widNum / 2.0; s < widNum / 2.0; s++) {
      for (int t = 0; t < heiNum; t++) {
        pt_random.x = _dynamic_obs_x + (r + 0.5) * _resolution + 1e-2;
        pt_random.y = _dynamic_obs_y + (s + 0.5) * _resolution + 1e-2;
        pt_random.z = _dynamic_obs_z + (t + 0.5) * _resolution + 1e-2;
        dynamic_cloudMap.points.push_back(pt_random);
      }
    }
  ...
  ROS_WARN("_total_time = %lf", _total_time);
  ROS_WARN("_delta_time = %lf", _delta_time);
  ROS_WARN("width = %d", dynamic_cloudMap.points.size());
  ROS_WARN("vel = %d", _dynamic_obs_vel);
  ROS_WARN("x = %lf", _dynamic_obs_x);
  ROS_WARN("y = %lf", _dynamic_obs_y);
  ROS_WARN("z = %lf", _dynamic_obs_z);
}
...
int main(int argc, char** argv) {
  ...
  _oneway_time = _y_size/_dynamic_obs_vel;
  _dynamic_obs_x = 0.0;
  _dynamic_obs_y = _y_l;
  _dynamic_obs_z = 0.5;
