...
pcl::PointCloud<pcl::PointXYZ> _all_cloud_data;
...
void rcvDynamicObsCallBack(const sensor_msgs::PointCloud2 & dynamic_obs_pointcloud)
{
  ...
  if(has_global_map) {
    ...
    // step 1: add static 
    for(int i = 0; i < _cloud_all_map.width; i++) {
      _all_cloud_data.points.push_back(_cloud_all_map.points[i]);
    }  
  }  
  // step 2: add dynamic
  for(int i = 0; i < int(dynamic_obs_cloudIn.points.size()); i++) {
    dynamic_obs_pt_in = dynamic_obs_cloudIn.points[i];
    _all_cloud_data.points.push_back(dynamic_obs_pt_in);

  }
  // printf("dynamic obs has points: %d.\n", (int)dynamic_obs_cloudIn.points.size());
  _kdtreeLocalMap.setInputCloud(_all_cloud_data.makeShared());
}
