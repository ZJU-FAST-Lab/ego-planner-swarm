...
namespace ego_planner
{
  ...
  void EGOReplanFSM::swarmTrajsCallback(const traj_utils::MultiBsplinesPtr &msg)
  {
    ...
    if ((int)msg->traj.size() == msg->drone_id_from + 1) // drone_id must start from 0
    {
      ...
      {
        bool success = planFromCurrentTraj(10);
        if (success)
        {
          changeFSMExecState(EXEC_TRAJ, "FSM");
        }
      ...
      // Step 3. Publish
      // Already published in function "bool EGOReplanFSM::callReboundReplan(bool, bool)"
    }
    else
    {
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    case INIT:
    {
      ...
      //   return;
      // }
      if (planner_manager_->pp_.drone_id >= 1 && !have_previous_drone_) // fuck
      {
        return;
      }
    ...
    case GEN_NEW_TRAJ:
    {
      ...
      bool success = planFromGlobalTraj(); // fuck
      if (success)
      {
        changeFSMExecState(EXEC_TRAJ, "FSM");
